import type { DtekResponse, DtekFact, HoursData } from "../types.js";
import {
...
const makeDtekResponse = (overrides: Partial<DtekResponse> = {}): DtekResponse => ({
  fact: {},
  preset: {},
  ...overrides,
});

const makeDtekFact = (overrides: Partial<DtekFact> = {}): DtekFact => ({
  ...overrides,
});
...
describe("helpers", () => {
  describe("getHouseDataFromResponse", () => {
    it("returns house data when valid response and house number exist", () => {
      const dtekResponse = makeDtekResponse({
        data: {
  ...
  describe("extractTodayUNIX", () => {
    it("returns UNIX timestamp when fact.today is a valid string number", () => {
      const fact = makeDtekFact({ today: "1704067200" });
    ...
    it("returns null when fact.today is zero", () => {
      const fact = makeDtekFact({ today: 0 });
    ...
    it("returns null when fact.today is not a number string", () => {
      const fact = makeDtekFact({ today: "invalid" });
      expect(extractTodayUNIX(fact)).toBeNull();
    });

    it("returns null when fact.today is a float", () => {
      const fact = makeDtekFact({ today: 1704067200.5 });
      expect(extractTodayUNIX(fact)).toBeNull();
    });

    it("returns null when fact is null", () => {
      expect(extractTodayUNIX(null as never)).toBeNull();
    });

    it("returns null when fact is undefined", () => {
      expect(extractTodayUNIX(undefined as never)).toBeNull();
    });

    it("returns null when fact.today is missing", () => {
      const fact = makeDtekFact();
      expect(extractTodayUNIX(fact)).toBeNull();
    });

    it("returns null when fact.today is an empty string", () => {
      const fact = makeDtekFact({ today: "" });
      expect(extractTodayUNIX(fact)).toBeNull();
    });
  });
  ...
  describe("getHoursData", () => {
    it("returns hours data when all parameters are valid", () => {
      const hoursData: HoursData = { "0": "yes", "1": "no", "2": "yes", "3": "no" };
      const fact = makeDtekFact({
        data: {
          "1704067200": { reason_123: hoursData },
        },
      });
    ...
    it("returns undefined when reasonKey does not exist", () => {
      const hoursData: HoursData = { "0": "yes" };
      const fact = makeDtekFact({
        data: {
          "1704067200": { reason_123: hoursData },
        },
